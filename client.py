    ...
    while True:
        ...
        if from_server.startswith("welcome"):
            ...
            try:
                ...
                chat_space.insert(tkinter.END, msg)
            ...
            # image / image1.gif
            #$opp_img_choice: {img_choice} pyimage2
        ...
        elif from_server.startswith("$opponent_choice"): #이거 시작이 이거면 안됨.
            ...
            threading._start_new_thread(count_down, (game_timer, "")) ##얘가 여기 있으면 안돼. 더 전에 게임 처음 시작할 때 실행 되어야 함.
    sck.close()
...
#frame3(대기방)
#선택 이미지 표시
select_char_can = tk.Label(frame3, width =150, height = 150) ##내 캐릭터 선택 결과 출력 위치
select_char_can.place(x=50, y=50)
# select_char_opp = tk.Label(frame3, width =150, height = 150) ##상대방 캐릭터 선택 결과 출력 위치
# select_char_opp.place(x=200, y=50)
...
#캐릭터 선택 버튼
char_select_br = tk.Button(frame3, text = "캐릭터 선택", command = char_select)
...
chamcham_st_bt= tk.Button(frame3,image=playbt,command=game_start)
